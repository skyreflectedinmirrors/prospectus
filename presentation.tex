...
\begin{frame}
 \frametitle{Challenges of reacting-flow simulations}
...
\begin{frame}
 ...
 \end{itemize}
 However, using even modestly sized chemical kinetic models with such a solver can incur severe computation cost (e.g.,~\footfullcite{Moiz2016123}) for realistic reacting-flow simulations.
\end{frame}
...
\begin{frame}
 ...
 \end{itemize}
 \textrightarrow Enter \texttt{pyJac-V2}\footfullcite{CURTIS2018186}
\end{frame}
...
\begin{columns}
 \begin{column}{0.5\textwidth}
  ...
  \begin{itemize}
   \item achievable CPU-speedup depends on vector-width of the device (\SI{4}{$\times$} here, up-to \SI{8}{$\times$} on newest processors).
  \end{itemize}
...
\begin{frame}
 ...
 Previous works demonstrated the value of GPU-based chemical kinetic integration for non-stiff to moderately-stiff chemistry (e.g.\footfullcite{2014NiemeyerGPU}$^,$\footfullcite{Shi:2012aa}), but GPU-integration of highly-stiff chemistry remained difficult due to issues with thread-divergence\footfullcite{Stone:2013aa}:
 \begin{itemize}
...
\begin{frame}
 \frametitle{Stiff GPU chemistry integration algorithms}
 The developed open-source\footnote{\url{https://github.com/SLACKHA/accelerInt}} integration algorithms were paired with an analytical Jacobian~\footfullcite{Niemeyer:2016aa}, and tested on chemical models of varying size, and levels of numerical stiffness, finding:%
 \vskip-1.5em
 \begin{columns}[t]
  \begin{column}{0.6\textwidth}
    \begin{itemize}
      \item Speedups for GPU-based implicit Runge--Kutta method, decreased with larger model sizes and global integration time-step size.\note[item]{i.e, with increasing stiffness}
      ...
      \item Up to \SI{241}{$\times$} speedup on GPU for analytical jacobian versus a finite-difference approach.
    \end{itemize}
  \end{column}
  \begin{column}{0.4\textwidth}
   \begin{figure}
...
\begin{frame}
 ...
 To demonstration computational efficiency and accuracy of the algorithms and codes developed thus far, they are applied to a bluff-body stabilized, turbulent reacting-flame, large eddy simulation in the OpenFOAM CFD code.
 \vskip-1.5em
 \begin{columns}[t]
  \begin{column}{0.6\textwidth}
   \begin{itemize}
    \item Selected case is a relatively simple configuration, well characterized with experimental velocity and temperature measurements available\footnotemark[1]
...
\begin{frame}
 ...
 Each time-step with the OpenFOAM solvers takes \textapprox\SI{3.5}{\hour}\footnote[2]{In large part due to poor chemistry load-balancing} on 92-cores (4-nodes) of UConn HPC.\note[item]{Intel\raisebox{1ex}{\scriptsize{\textregistered}} Xeon\raisebox{1ex}{\scriptsize{\textregistered}} E5-2690 v3 Processor}
\end{frame}
